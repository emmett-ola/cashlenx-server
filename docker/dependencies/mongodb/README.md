...
API container. Start attaches MongoDB to `DOCKER_NETWORK_NAME`; stop removes it
only if no other container remains attached. `MONGO_PROJECT_NAME` and
`MONGO_CONTAINER_NAME` explicitly control its Compose and container identities.
