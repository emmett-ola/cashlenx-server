...
func SaveIncome(belongsDate, categoryName string, amount float64, description string, userId string) (model.CashFlowEntity, error) {
	...
	if !strings.EqualFold(categoryEntity.Type, model.FlowTypeIncome) {
		return model.CashFlowEntity{}, errors.New("category type mismatch: expected INCOME category")
	}
	...
	newCashFlowId := cash_flow_mapper.INSTANCE.InsertCashFlowByEntity(model.CashFlowEntity{
		...
		BelongsDate: date,
		// FlowType:    model.FlowTypeIncome, // Deprecated
		Amount:      amount,
