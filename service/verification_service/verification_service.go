...
func GenerateVerificationToken(userId string, operation OperationType, payload string) (string, error) {
	...
	err := operation_confirm_code_mapper.INSTANCE.InvalidateActiveCodes(userId, string(operation))
	if err != nil {
	...
	// Store in database
	err = operation_confirm_code_mapper.INSTANCE.CreateCode(code)
	if err != nil {
		return "", err
	}
...
func VerifyToken(token string, operation OperationType) (string, string, bool) {
	code := operation_confirm_code_mapper.INSTANCE.GetCodeByToken(token)
	...
	if time.Now().After(code.ExpiresTime) {
		...
		operation_confirm_code_mapper.INSTANCE.DeleteCode(code.Id)
		return "", "", false
	}
...
func InvalidateToken(token string) {
	code := operation_confirm_code_mapper.INSTANCE.GetCodeByToken(token)
	if code.Id != "" {
		operation_confirm_code_mapper.INSTANCE.MarkCodeAsUsed(code.Id)
	}
}
...
func InvalidateTokensByUserAndOperation(userId string, operation OperationType) {
	operation_confirm_code_mapper.INSTANCE.InvalidateActiveCodes(userId, string(operation))
}
