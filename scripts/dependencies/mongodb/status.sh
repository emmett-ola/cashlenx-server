...
diagnose_container "$(read_config_value MONGO_CONTAINER_NAME cashlenx-mongodb)" \
  "$MONGO_IMAGE" "$(resolve_network_name)" \
  sh -ec 'test "$(cat /proc/1/comm)" = mongod && mongosh --quiet --username "$MONGO_INITDB_ROOT_USERNAME" --password "$MONGO_INITDB_ROOT_PASSWORD" --authenticationDatabase admin --eval "db.runCommand({ ping: 1 }).ok" "127.0.0.1:${MONGO_CONTAINER_PORT:-27017}/${MONGO_INITDB_DATABASE}" >/dev/null'
