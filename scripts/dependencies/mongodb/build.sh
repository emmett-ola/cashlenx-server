...
# MongoDB is an upstream image, so the dependency build step prepares it by
# pulling the configured image. Build-only values allow image preparation before
# deployment credentials are filled in and are never written to the env file.
MONGO_ROOT_USERNAME=dependency-build MONGO_ROOT_PASSWORD=dependency-build \
  docker compose --env-file "$env_file" -f "$compose_file" pull mongodb
